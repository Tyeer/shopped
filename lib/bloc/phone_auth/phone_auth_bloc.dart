...
class PhoneAuthBloc extends Bloc<PhoneAuthEvent, PhoneAuthState> {
  final Repository phoneAuthRepository;
  ...
  Future<void> _loginWithCredential(OnPhoneAuthVerificationCompleteEvent event,
    ...
    UserCredential user = await auth.signInWithCredential(event.credential);
    Seller? verifySeller = await phoneAuthRepository
        .findSellerByPhoneNumber(user.user?.phoneNumber);
    try {
      if (user.user != null) {
        if (verifySeller != null) {
          prefs.setString('userId', user.user!.uid);
          prefs.setString('userRole', 'seller');
          prefs.setString('phoneNumber', user.user?.phoneNumber ?? "");
          debugPrint("seller-old");
          emit(PhoneAuthVerified(status: "Seller-OldAccount"));
        }
        else {
          if (await phoneAuthRepository
              .findBuyerByPhoneNumber(user.user?.phoneNumber) !=
              null) {
            //save the user id to the local storage
            prefs.setString('userId', user.user!.uid);
            prefs.setString('userRole', 'buyer');
            prefs.setString('phoneNumber', user.user?.phoneNumber ?? "");
            debugPrint("buyer-old");
            // log in as buyer
            emit(PhoneAuthVerified(status: "Buyer-OldAccount"));
          }
          else {
            prefs.setString('userId', user.user!.uid);
            prefs.setString('userRole', 'buyer');
            prefs.setString('phoneNumber', user.user?.phoneNumber ?? "");
            debugPrint("buyer-new");
            // log in as buyer
            emit(PhoneAuthVerified(status: "Buyer-NewAccount"));
          }
        }
  ...
  Future<void> _logOut(LogOutEvent event, Emitter<PhoneAuthState> emit) async {
    final prefs = await SharedPreferences.getInstance();
    prefs.clear();
    auth.signOut();
  }
